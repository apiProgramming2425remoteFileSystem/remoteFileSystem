...
impl PathFilesystem for Fs {
    ...
    ****************/
    ...
    async fn getattr(
        ...
        let attributes = self
            .fs
            .get_attributes(path_osString.as_os_str() /* , &self.auth_token */)
            .await
            .map_err(|err| {
                tracing::error!("{}", err);
                libc::ENOENT
    ...
    async fn statfs(&self, req: Request, path: &OsStr) -> FuseResult<ReplyStatFs> {
        ...
        let stats = self
            .fs
            .get_fs_stats(&path /* , &self.auth_token */)
            .await
            .map_err(|err| {
                tracing::error!("{}", err);
                libc::ENOENT
    ...
    async fn access(&self, req: Request, path: &OsStr, mask: u32) -> FuseResult<()> {
        ...
        let permissions = self
            .fs
            .get_permissions(&path /* , &self.auth_token */)
            .await
            .map_err(|err| {
                tracing::error!("{}", err);
                libc::ENOENT
    ...
    async fn mkdir(
        ...
        self.fs
            .mkdir(complete_path.as_os_str() /* , &self.auth_token */)
            .await
            .map(|attr| ReplyEntry {
                ttl: TTL,
                attr: attr.into(),
    ...
    async fn rmdir(&self, req: Request, parent: &OsStr, name: &OsStr) -> FuseResult<()> {
        ...
        match self
            .fs
            .remove(path.as_os_str() /* , &self.auth_token */)
            .await
        {
            Ok(()) => Ok(()),
    ...
    async fn rename(
        ...
        match self
            ...
            .rename(
                old_path.as_os_str(),
                new_path.as_os_str(), /* , &self.auth_token */
            )
            .await
        {
            Ok(_) => Ok(()),
