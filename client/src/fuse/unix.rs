...
impl PathFilesystem for Fs {
    ...
    async fn getattr(
        ...
        let attributes = self
            .fs
            .get_attributes(path_osString.as_os_str())
            .await
            .map_err(|err| {
                tracing::error!("{}", err);
                libc::ENOENT
    ...
    async fn readdir<'a>(
        ...
        let entries: Vec<Result<DirectoryEntry>> = items
            .into_iter()
    ...
    async fn readdirplus<'a>(
        ...
        let entries: Vec<Result<DirectoryEntryPlus>> = items
            .into_iter()
            .skip(offset as usize)
            .enumerate()
            .map(|(idx, item)| {
...
impl From<fuse3::SetAttr> for fs_model::SetAttr {
    fn from(value: fuse3::SetAttr) -> Self {
        Self {
            ...
            atime: value.atime.map(fs_model::Timestamp::from),
            mtime: value.mtime.map(fs_model::Timestamp::from),
            ctime: value.ctime.map(fs_model::Timestamp::from),
        }
    }
}
