...
impl LogLevel {
    pub fn as_str(&self) -> &'static str {
        match self {
            LogLevel::Trace => "backend=trace,actix_web=info",
            LogLevel::Debug => "backend=debug,actix_web=info",
            LogLevel::Info => "backend=info,actix_web=info",
            LogLevel::Warn => "backend=warn,actix_web=info",
            LogLevel::Error => "backend=error,actix_web=info",
        }
    }
}
