...
async fn test_delete_file() -> anyhow::Result<()> {
    ...
    // Cancella file
    client
        .delete(&client.set_url("files", "/d/f.txt"))
        .send()
        .await?
        .error_for_status()?;
...
async fn test_delete_file_and_directory() -> anyhow::Result<()> {
    ...
    // crea cartella + file
    let _ = client
        .post(&client.set_url("mkdir", "/tmpDir"))
        .send()
        .await?;
    ...
    // cancella file
    let resp = client
        .delete(&client.set_url("files", "/tmpDir/file.txt"))
        .send()
        .await?;
    assert!(resp.status().is_success());
    ...
    // cancella cartella
    let resp = client
        .delete(&client.set_url("files", "/tmpDir"))
        .send()
        .await?;
    assert!(resp.status().is_success());
...
async fn write_with_offset_beyond_eof_pads_with_zeros() -> anyhow::Result<()> {
    ...
    // write at offset 10
    let write_url = format!("{}?offset=10", client.set_url("files", "/c.txt"));
...
async fn rename_exchange_swaps_source_and_target() -> anyhow::Result<()> {
    ...
    client
        .post(&client.set_url("mkdir", "/A"))
        .send()
        .await?
        .error_for_status()?;
    client
        .post(&client.set_url("mkdir", "/B"))
        .send()
        .await?
        .error_for_status()?;
