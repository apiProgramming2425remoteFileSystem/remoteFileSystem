...
impl FileSystem {
    ...
    pub fn move_node<P: AsRef<Path> + Debug>(&self, old_path: P, new_path: P) -> Result<()> {
        ...
        {
            return Err(StorageError::InvalidPath("Cannot move a directory inside itself or its descendants".into()));
        }
        ...
        let old_real_path = self.make_real_path(node_to_move.clone())?;

        // moving in the in-memory file system
        let parent_new = self
        ...
        parent_new_guard.add(node_to_move.clone());

        // moving in the real fs
        let new_real_path = self.make_real_path(node_to_move.clone())?;
